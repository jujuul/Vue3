...
export default defineComponent({
   ...
   */
  setup() {
    // const obj: any = { // 为了在使用obj.gender='男'时不出现错误信息才这么写的
    const obj = {
      name: "小明",
      age: 20,
      wife: {
    ...
    // function updateUser(){}
    const updateUser = () => {
      // 直接使用目标对象的方式来更新目标对象中成员的值是不可行的，只能使用代理对象的方式来更新数据（响应式数据）
      // obj.name = '小红' // 不行
      // user.name = "小红";
      // user.age += 2;
      // user.wife.name += "大甜甜";
      ...
      // 总结：如果操作代理对象，目标对象中的数据也会发生变化，如果想要在操作数据的时候，界面也要跟着重新更新渲染，那么也是操作代理对象
